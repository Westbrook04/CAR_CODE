...
void Servo_init_angle_adjust_mode_check(void)
{
	static int check_count1=0,check_count2=0;
		...
		if(PS2_LX<100 && PS2_RX>200)   check_count1++,Move_X=0,Move_Y=0,Move_Z=0;  //手柄左右两边摇杆往左右两边推,底盘运动量控制为0，防止小车乱走
	  else check_count1=0;
	
		if(PS2_LX>200 && PS2_RX<100)   check_count2++,Move_X=0,Move_Y=0,Move_Z=0;  //手柄左右两边摇杆往中间推,底盘运动量控制为0，防止小车乱走
	  else check_count2=0;
	//进入模式检测
	if(check_count1>500)//连续推住5秒
	{
		
		Beep_Control_Flag = 1;
		Servo_init_angle_adjust=1; //进入舵机初始位置调整模式
		check_count1=0;
		
	}
		//退出模式检测
	if(check_count2>500 && Servo_init_angle_adjust==1)//连续推住5秒
	{
		Beep_Control_Flag = 1;
		Servo_init_angle_adjust=0;
	  Flash_Write();     //将调整好的值写入flash
		check_count2=0;
	}
