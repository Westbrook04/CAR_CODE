	...
	{	      
      ...
      // Enter the APP direction control interface		
		...
		else if(Turn_Flag==1)
		{
			...
			if     (Usart_Receive==0x41) car_A_steer_flag=1;//小车控制
			if     (Usart_Receive==0x45)	car_A_steer_flag=2;//舵机控制
			else  Flag_Direction=0;
		}
	 ...
	 if(Flag_PID==2) //Analyze the data //分析数据
	 {
      ...
      {								
				...
				switch(Receive[1])
				 {
					 case 0x30:  Moveit_Angle1=Data*0.01f-1.57f;break; //*0.01是因为参数在发送前是放大了一百倍的，这里需要做还原
					 case 0x31:  Moveit_Angle2=Data*0.01f-1.57f;break; //-1.57是因为参数在发送前加了1.57，这里需要做还原
